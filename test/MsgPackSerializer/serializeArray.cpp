...
static void check(const JsonArrayRef array, const char* expected_data,
                  size_t expected_len) {
...
template <size_t N>
static void check(const JsonArrayRef array, const char (&expected_data)[N]) {
  const size_t expected_len = N - 1;
  check(array, expected_data, expected_len);
}

// TODO: this function is used by the commented test
// static void check(const JsonArrayRef array, const std::string& expected) {
