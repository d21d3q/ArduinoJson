...
static void check(const JsonObjectRef object, const char* expected_data,
                  size_t expected_len) {
...
template <size_t N>
static void check(const JsonObjectRef object, const char (&expected_data)[N]) {
  const size_t expected_len = N - 1;
  check(object, expected_data, expected_len);
}

// TODO: used by the commented test
// static void check(const JsonObjectRef object, const std::string& expected) {
